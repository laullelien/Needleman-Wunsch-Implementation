...
Il est donc impossible d'allouer un espace mémoire aussi grand, ce qui explique l'erreur.
\begin{verbatim}
...
On utilise 2 tableaux de taille $\min(M, N) + 1$. Pour la suite, on suppose que $N = \min(M, N)$, ce qui peut être obtenu en échangeant $X$ et $Y$. Une ligne permet de mémoriser les valeurs de $\phi(i, j)$ pour un certain $i$ et $j$ tel que $0 \leq j \leq N$. On peut calculer la ligne suivante à partir de la précédente puis échanger les deux. On réitère jusqu'à avoir calculé toutes les lignes et on renvoie le résultat.
...
On utilise le même principe que pour l'algorithme itératif. Cependant, nos 2 tableaux ne sont plus de taille N + 1 mais d'une taille T qui permet que les 2 tableaux rentrent dans le cache. On ne peut donc plus calculer une ligne entière ($\phi(i,j)$ pour un certain $i$ et $j$ tel que $0 \leq j \leq N$). On peut cependant calculer $phi(i, j)$ pour un certain i et pour T valeurs de $j$.
On mémorise ensuite ces résultats intermédiaires dans un tableau memo de taille M puis on réitère l'opération en utilisant memo. Dans la suite, M est la taille de la plus grande chaîne de caractères.
