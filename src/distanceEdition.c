 ...
 * \brief compute edit distance between two strings each one being a substring
of consecutive characters (eg in a FNA file)
 * \version 0.1
 * \date 30/09/2022
 * \author Jean-Louis Roch (Ensimag, Grenoble-INP, University Grenoble-Alpes)
jean-louis.roch@grenoble-inp.fr
 * Usage : distanceEdition file1 b1 L1 file2 b2 L2
 * cf function usage_and_spec below.
NAME
     distanceEdition - compute edit distance between two substrings, each from a
file SYNOPSIS distanceEdition file_1 b1 L_1 file_2 b_2 L_2 DESCRIPTION
     ...
     array_F[b,b+L( : denotes in file F the sequence of L consecutive characters
starting at position b. The progam is designed for direct access to genetic
subsequences stored in FASTA format file (.fna) . Execution performs the
following actions:
     1. maps file_1 in an array array_file_1 in virtual memory using standard C
libray function: mmap
     2. prints on stderr if (L_1 <= 40)  the content of array_file_1[b_1 ..
b_1+L_1-1] or else the first and last 20 characters of array_file_1
     3. if file_2 and file_1 are the same pathname, than array_file_2 =
array_file_1; else do for file_2 the same as file_1 (replacing _1 by _2 in both
...
EXAMPLE
    ...
    where f1.fna contains =\"CAcgT\"
      and f2.fna contains \"<cette premiere ligne est un
commentaire.\\nacaCGTACNNNAT\" \n" prints the output of editDistance on the two
following char arrays :
...
/**
 * \file distanceEdition.c
 * \brief Primitives pour mapper en mémoire virtuelle une sous-séquence d'un
 * fichier de caractères \author Jean-Louis Roch jean-louis.roch@grenoble-inp
 * \date 30/10/2012
 * \brief Primitives pour mapper en mémoire virtuelle une sous-séquence d'un
...
{
    fprintf(stderr,
            "%s : bad number of arguments: 6 are required (but this execution "
            "is with %d instead).\n"
            "Usage:   %s  file_1 begin_1 length_1 file_2 begin_2 length_2 \n\n"
            "%s prints the edit distance between two genetic sequences seq[i] "
            "for i=1..2  where \n"
            "seq[i] denotes the sequence of <length_i> char in <file_i> from "
            "position <begin_i>.",
    ...
    fprintf(stderr,
            "\n"
            "\nNAME"
            "\n     distanceEdition - compute edit distance between two "
            "substrings, each from a file"
            "\nSYNOPSIS"
            "\n     distanceEdition file_1 b1 L_1 file_2 b_2 L_2"
            "\nDESCRIPTION"
            "\n     distanceEdition computes the edit distance between two "
            "arrays of"
            "\n     characters array_file_1[b_1, b_1+L_1( and "
            "array_file2[b_2,b_2+L_2( where:"
            "\n     array_F[b,b+L(  denotes in file F the sequence of L "
            "consecutive characters starting at position b."
            "\n     The progam is designed for direct access to genetic "
            "subsequences stored in FASTA format file (.fna) ."
            "\n     Execution performs the following actions:"
            "\n     1. maps file_1 in an array array_file_1 in virtual memory "
            "using standard C libray function: mmap"
            "\n     2. prints on stderr if (L_1 <= 40)  the content of "
            "array_file_1[b_1 .. b_1+L_1-1]"
            "\n        or else the first and last 20 characters of array_file_1"
            "\n     3. if file_2 and file_1 are the same pathname, than "
            "array_file_2 = array_file_1;"
            "\n        else do for file_2 the same as file_1 (replacing _1 by "
            "_2 in both previous actions above)"
            "\n     4. prints on stdout the output (type long) of the "
            "following call :"
            "\n           editDistance( array_file_1 + b_1, L_1, array_file_2, "
            "+ b_2, L_2 )"
            "\n        where the extern C function has prototype :"
            "\n           editDistance( char* A, size_t lengthA, char* B, "
            "size_t lengthB);"
            "\nEXIT STATUS"
            "\n     The program exits 0 on success, and >0 if an error occurs."
            "\nEXAMPLE"
            ...
            "\n      and f2.fna contains \"<cette premiere ligne est un "
            "commentaire.\\nacaCGTACNNNAT\" \n"
            "\n    prints the output of editDistance on the two following char "
            "arrays :"
            "\n    {'C', 'A', 'c', 'G', 'T'} extracted from f1.fna"
            "\n    {'a', 'c', 'a', 'C', 'G', 'T', 'A'} extracted from f2.fna"
            "\n    and prints 4 on stdout."
...
int main(int argc, char *argv[]) {
         ...
         ++i, argv += 3) // defines content and length of seq[i] for i=0..1
    {
        ...
        if (fstat(fd[i], &s) == -1)
            err(1, "fstat");
        mmap_fd[i] =
            (char *)mmap(NULL, s.st_size, PROT_READ, MAP_PRIVATE, fd[i], 0);
        if (mmap_fd[i] == MAP_FAILED)
            err(1, "mmap");
        mmap_length[i] = (long)s.st_size;

        { // Assign seq[i] to the begining of the sequence, excluding comment
          // lines starting by '<'
            long debut;
            ...
            if (*seq[i] ==
                '>') /* Skip and print the first line starting by '<' */
            {
                char *endofline = strchr(seq[i], '\n');
                fprintf(stderr, "Sequence comment in preamble: ");
                for (char *c = seq[i]; c <= endofline; ++c)
                    fprintf(stderr, "%c", *c);
                if (*endofline == '\n')
    ...
    // par exemple p.cumul_energy donne le nombre de kWh consommés sur le
    // processeur entre l'appel à perf_start et celui à perfstop_and_display
#endif
