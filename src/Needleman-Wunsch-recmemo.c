 ...
 * \brief recursive implementation with memoization of Needleman-Wunsch global
 * alignment algorithm that computes the distance between two genetic sequences
 * \version 0.1
 * \date 03/10/2022
 ...
 * Costs of basic base opertaions (SUBSTITUTION_COST, SUBSTITUTION_UNKNOWN_COST,
 * INSERTION_COST) are defined in Needleman-Wunsch-recmemo.h
 */
 ...
 * \brief default value for memoization of minimal distance (defined as an
 * impossible value for a distance, -1).
 */
...
struct NW_MemoContext {
    ...
    long **memo; /*!< memoization table to store memo[0..M][0..N] (including
                    stopping conditions phi(M,j) and phi(i,N) */
};

/*
 *  static long EditDistance_NW_RecMemo(struct NW_MemoContext *c, size_t i,
 * size_t j)
 * \brief  EditDistance_NW_RecMemo :  Private (static)  recursive function with
 * memoization \ direct implementation of Needleman-Wursch extended to manage
 * FASTA sequences (cf TP description) \param c : data passed for recursive
 * calls that includes the memoization array \param i : starting position of the
 * left sequence :  c->X[ i .. c->M ] \param j : starting position of the right
 * sequence :  c->Y[ j .. c->N ]
 */
static long EditDistance_NW_RecMemo(struct NW_MemoContext *c, size_t i,
                                    size_t j)
/* compute and returns phi(i,j) using data in c -allocated and initialized by
   EditDistance_NW_Rec */
{
 ...
 * It allocates and initailizes data (NW_MemoContext) for memoization and call
 * the recursivefunction EditDistance_NW_RecMemo See .h file for documentation
 */
long EditDistance_NW_Rec(char *A, size_t lengthA, char *B, size_t lengthB) {
    ...
    { /* Allocation and initialization of ctx.memo to NOT_YET_COMPUTED*/
        /* Note: memo is of size (N+1)*(M+1) but is stored as (M+1) distinct
         * arrays each with (N+1) continuous elements It would have been
         * possible to allocate only one big array memezone of (M+1)*(N+1)
    ...
    /* Compute phi(0,0) = ctx.memo[0][0] by calling the recursive function
     * EditDistance_NW_RecMemo */
